...
sed -i -e "s/VERSION/%VERSION%/" %ROOT_DIR%\cli\ci\scripts\windows-installer.iss
sed -i -e "s/CF_SOURCE/%ESCAPED_ROOT_DIR%\\cf.exe/" %ROOT_DIR%\cli\ci\scripts\windows-installer.iss
...
ISCC %ROOT_DIR%\cli\ci\scripts\windows-installer.iss

MOVE %ROOT_DIR%\cli\ci\scripts\Output\setup.exe cf_installer.exe
