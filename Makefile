...
units-plugin:
	CF_HOME=$(PWD)/fixtures $(ginkgo_units) -nodes 1 --flake-attempts 2 --skip-package integration ./**/plugin* ./plugin
...
units-non-plugin:
	...
	CF_HOME=$(PWD)/fixtures $(ginkgo_units) --flake-attempts 3 cf/ssh
