...
func TestDecodeTokenInfoWithoutRestoringPadding(t *testing.T) {
	...
	decodedInfo, err := DecodeTokenInfo(accessToken)

	assert.NoError(t, err)
	assert.Contains(t, string(decodedInfo), "user1@example.com")
}

func TestDecodeTokenInfoWhenRestoringPadding(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Contains(t, string(decodedInfo), "tlang@gopivotal.com")
}
