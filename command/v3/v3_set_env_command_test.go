		...
		Context("when getting the current user succeeds", func() {
			...
			Context("when setting the environment succeeds", func() {
				...
				It("sets the environment variable and value pair", func() {
					...
					Expect(testUI.Out).To(Say("Setting env variable 'some-key' to 'some-value' for app some-app in org some-org / space some-space as banana\\.\\.\\."))

					Expect(testUI.Err).To(Say("set-warning-1"))
					Expect(testUI.Err).To(Say("set-warning-2"))
					Expect(testUI.Out).To(Say("OK"))
					Expect(testUI.Out).To(Say("TIP: Use 'cf v3-restage some-app' to ensure your env variable changes take effect\\."))
