...
var _ = Describe("Enable Feature Flag Command", func() {
	var (
		...
		fakeActor       *v7fakes.FakeEnableFeatureFlagActor
		executeErr      error
		binaryName      string
	...
	BeforeEach(func() {
		...
		fakeActor = new(v7fakes.FakeEnableFeatureFlagActor)
	...
	When("the environment is setup correctly", func() {
		...
		It("should print text indicating its running", func() {
			...
			Expect(testUI.Out).To(Say(`Enabling feature flag flag1 as apple\.\.\.`))
		})

		When("updating featureFlag fails", func() {
			BeforeEach(func() {
				fakeActor.EnableFeatureFlagReturns(v7action.Warnings{"this is a warning"},
		...
		When("updating featureFlag succeeds", func() {
			...
			When("there is no featureFlag", func() {
				BeforeEach(func() {
					fakeActor.EnableFeatureFlagReturns(v7action.Warnings{"this is a warning"}, actionerror.FeatureFlagNotFoundError{FeatureFlagName: featureFlagName})
				})
