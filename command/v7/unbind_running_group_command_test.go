...
var _ = Describe("unbind-running-security-group Command", func() {
	var (
		cmd             UnbindRunningSecurityGroupCommand
		testUI          *ui.UI
	...
	When("the command executes successfully", func() {
		...
		It("unbinds the security group from being available globally for running", func() {
			Expect(executeErr).NotTo(HaveOccurred())
			Expect(testUI.Out).To(Say("Unbinding security group %s from defaults for running as some-user...", cmd.RequiredArgs.SecurityGroup))
