...
type SpaceQuotaActor interface {
	GetSpaceQuotaByName(spaceQuotaName string) (v7action.SpaceQuota, v7action.Warnings, error)
}
...
func (cmd SpaceQuotaCommand) Execute(args []string) error {
	...
	cmd.UI.DisplayTextWithFlavor(
		"Getting space quota {{.QuotaName}} info as {{.Username}}...",
	...
	orgQuota, warnings, err := cmd.Actor.GetSpaceQuotaByName(quotaName)
