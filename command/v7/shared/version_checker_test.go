	...
	Context("CheckCCAPIVersion", func() {
		...
		When("the current version is less than the minimum version", func() {
			...
			It("does return a warning", func() {
				Expect(executeErr).ToNot(HaveOccurred())
				Expect(warning).To(Equal("\nWarning: Your targeted API's version (3.83.0) is less than the minimum supported API version (3.160.0). Some commands may not function correctly."))
			})
		})
