...
func sortUsers(usersWithRole []v7action.User) {
	sort.Slice(usersWithRole, func(i, j int) bool {
		if usersWithRole[i].PresentationName == usersWithRole[j].PresentationName {
			...
			return usersWithRole[i].Origin < usersWithRole[j].Origin
		}

		return usersWithRole[i].PresentationName < usersWithRole[j].PresentationName
	})
}

func getOrigin(user v7action.User) string {
	if user.Origin == "" {
		return "client"
	}
