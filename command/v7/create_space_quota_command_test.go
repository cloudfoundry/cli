	...
	When("the environment is setup correctly", func() {
		...
		When("no flag limits are given", func() {
			...
			It("creates the space quota in the targeted organization", func() {
				...
				Expect(expectedLimits).To(Equal(v7action.SpaceQuotaLimits{}))
			})
		...
		When("all flag limits are given", func() {
			BeforeEach(func() {
				cmd.TotalMemory = flag.MegabytesUnlimited{
					NullInt: types.NullInt{IsSet: true, Value: 47},
				}

				cmd.PerProcessMemory = flag.MegabytesUnlimited{
					NullInt: types.NullInt{IsSet: true, Value: 23},
				}

				cmd.NumAppInstances = flag.IntegerLimit{
					NullInt: types.NullInt{IsSet: true, Value: 4},
				}

				cmd.PaidServicePlans = true

				cmd.TotalServiceInstances = flag.IntegerLimit{
					NullInt: types.NullInt{IsSet: true, Value: 9},
				}

				cmd.TotalRoutes = flag.IntegerLimit{
					NullInt: types.NullInt{IsSet: true, Value: 1},
				}

				cmd.TotalReservedPorts = flag.IntegerLimit{
					NullInt: types.NullInt{IsSet: true, Value: 7},
				}
		...
		When("the space quota already exists", func() {
			BeforeEach(func() {
				fakeActor.CreateSpaceQuotaReturns(v7action.Warnings{"some-warning"}, actionerror.SpaceQuotaAlreadyExistsError{SpaceQuota: spaceQuotaName})
			})
			...
			It("displays all warnings, that the space quota already exists, and does not error", func() {
				...
				Expect(testUI.Out).To(Say(`Creating space quota %s for org %s as %s\.\.\.`, spaceQuotaName, "some-org-name", userName))
				Expect(testUI.Err).To(Say(`Space quota '%s' already exists\.`, spaceQuotaName))
				Expect(testUI.Out).To(Say("OK"))
			})
		})
	})
