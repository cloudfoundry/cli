...
var _ = Describe("feature flags Command", func() {
	var (
		...
		fakeActor       *v6fakes.FakeFeatureFlagsActor
		binaryName      string
		executeErr      error
	)
	...
	BeforeEach(func() {
		...
		fakeActor = new(v6fakes.FakeFeatureFlagsActor)
	...
	When("the user is logged in", func() {
		...
		When("getting the current user succeeds", func() {
			...
			When("there are no feature flags", func() {
				BeforeEach(func() {
					fakeActor.GetFeatureFlagsReturns(
						[]v2action.FeatureFlag{},
						v2action.Warnings{"get-flags-warning"},
			...
			When("there are feature flags", func() {
				BeforeEach(func() {
					fakeActor.GetFeatureFlagsReturns(
						[]v2action.FeatureFlag{
							{
