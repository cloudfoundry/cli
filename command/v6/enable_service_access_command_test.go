		...
		When("the user is logged in", func() {
			...
			When("no flags are passed", func() {
				...
				When("enabling access to the service fails", func() {
					BeforeEach(func() {
						fakeActor.EnableServiceForAllOrgsReturns(v2action.Warnings{"warning"}, errors.New("explode"))
					})
			...
			When("the -p flag is passed", func() {
				BeforeEach(func() {
					servicePlan := "some-plan"
					cmd.ServicePlan = servicePlan
				...
				It("outputs a unique header", func() {
				...
				When("enabling access to the plan fails", func() {
					BeforeEach(func() {
						fakeActor.EnablePlanForAllOrgsReturns(v2action.Warnings{"warning"}, errors.New("explode"))
					})
			...
			When("the -o flag is passed", func() {
				...
				When("enabling access to the org fails", func() {
					BeforeEach(func() {
						fakeActor.EnableServiceForOrgReturns(v2action.Warnings{"warning"}, errors.New("explode"))
					})
			...
			When("the -p and -o flags are passed", func() {
				...
				When("enabling access to the plan in the org fails", func() {
					BeforeEach(func() {
						fakeActor.EnablePlanForOrgReturns(v2action.Warnings{"warning"}, errors.New("explode"))
					})
