	...
	Describe("help", func() {
		Context("when --help flag is set", func() {
			It("Displays command usage to output", func() {
				...
				Eventually(session).Should(Say("v3-push - Push a new app or sync changes to an existing app"))
				Eventually(session).Should(Say("USAGE:"))
				Eventually(session).Should(Say("cf v3-push APP_NAME \\[-b BUILDPACK\\]\\.\\.\\. \\[-p APP_PATH\\] \\[--no-route\\]"))
				Eventually(session).Should(Say("cf v3-push APP_NAME --docker-image \\[REGISTRY_HOST:PORT/\\]IMAGE\\[:TAG\\] \\[--docker-username USERNAME\\] \\[--no-route\\]"))
	...
	Context("when the environment is set up correctly", func() {
		...
		Context("when the app exists", func() {
			...
			It("pushes the app", func() {
				...
				Eventually(session).Should(Say("memory usage:\\s+\\d+M x 1"))
		...
		Context("when the app crashes", func() {
			...
			It("pushes the app", func() {
				...
				Eventually(session).Should(Say("name:\\s+%s", appName))
				Eventually(session).Should(Say("requested state:\\s+started"))
				Eventually(session).Should(Say("processes:\\s+web:0/1"))
				Eventually(session).Should(Say("memory usage:\\s+\\d+M x 1"))
				Eventually(session).Should(Say("routes:\\s+%s\\.%s", appName, domainName))
				Eventually(session).Should(Say("stack:\\s+cflinuxfs2"))
				Eventually(session).Should(Say("buildpacks:\\s+ruby"))
				Eventually(session).Should(Say(""))
				Eventually(session).Should(Say("web:0/1"))
				Eventually(session).Should(Say(`state\s+since\s+cpu\s+memory\s+disk`))
