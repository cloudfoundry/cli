...
var _ = Describe("bind-staging-security-group command", func() {
	...
	When("the environment is not setup correctly", func() {
		It("fails with the appropriate errors", func() {
			helpers.UnrefactoredCheckEnvironmentTargetedCorrectly(false, false, ReadOnlyOrg, "bind-staging-security-group", secGroupName)
		})
	})
	...
	When("the security group doesn't exist", func() {
		It("fails with a security group not found message", func() {
			...
			Eventually(session).Should(Say("security group some-security-group-that-doesn't-exist not found"))
			Eventually(session).Should(Exit(1))
		})
	})
