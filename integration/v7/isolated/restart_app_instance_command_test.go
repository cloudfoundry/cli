...
var _ = Describe("restart-app-instance command", func() {
	...
	When("--help flag is set", func() {
		It("Displays command usage to output", func() {
			...
			Eventually(session).Should(Say(`--process\s+Process to restart \(Default: web\)`))
	...
	When("the app name is not provided", func() {
		It("tells the user that the app name is required, prints help text, and exits 1", func() {
			session := helpers.CF("restart-app-instance")
	...
	When("the index is not provided", func() {
		It("tells the user that the index is required, prints help text, and exits 1", func() {
			session := helpers.CF("restart-app-instance", appName)
	...
	When("the environment is not setup correctly", func() {
		When("no API endpoint is set", func() {
			...
			It("fails with no API endpoint set message", func() {
				session := helpers.CF("restart-app-instance", appName, "1")
	...
	When("the environment is setup correctly", func() {
		...
		When("app does not exist", func() {
			It("fails with error", func() {
				session := helpers.CF("restart-app-instance", appName, "0", "--process", "some-process")
		...
		When("app exists", func() {
			...
			When("process type is not provided", func() {
				It("defaults to web process", func() {
					...
					session := helpers.CF("restart-app-instance", appName, "0")
			...
			When("a process type is provided", func() {
				When("the process type does not exist", func() {
					It("fails with error", func() {
						session := helpers.CF("restart-app-instance", appName, "0", "--process", "unknown-process")
				...
				When("the process type exists", func() {
					When("instance index exists", func() {
						...
						It("defaults to requested process", func() {
							...
							session = helpers.CF("restart-app-instance", appName, "0", "--process", "console")
					...
					When("instance index does not exist", func() {
						It("fails with error", func() {
							session := helpers.CF("restart-app-instance", appName, "42", "--process", constant.ProcessTypeWeb)
