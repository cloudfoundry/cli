...
//TODO: Should we add some assertions here based on the output of `cf org-users` before and after?
// i.e. Should we assert on the absence of a deleted role when we look at all of the output of `cf org-users`?
// Note that the story hints at this.
var _ = Describe("unset-org-role command", func() {
	...
	When("logged in as a privileged user", func() {
		When("the --client flag is passed", func() {
			...
			When("the client exists", func() {
				...
				//TODO: Do we want to keep this test here, given that the `unset-space-role` version of this test was
		...
		FWhen("the user does not exist", func() {
			It("prints an appropriate error and exits 1", func() {
	...
	When("the logged in user does not have permission to write to the org", func() {
		...
		It("prints out the error message from CC API and exits 1", func() {
			...
			// OR if we just surface the CAPI error here, the expectation would read:
			//TODO: Align this with whatever unset-space-role does.
