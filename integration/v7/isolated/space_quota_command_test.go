...
var _ = FDescribe("space-quota command", func() {
	var (
		quotaName string
		orgName   string
	)
	...
	When("the environment is set up correctly", func() {
		...
		When("the quota does not exist", func() {
			It("displays quota not found and exits 1", func() {
				...
				Eventually(session).Should(Say(`Getting space quota %s info as %s\.\.\.`, quotaName, userName))
				Eventually(session.Err).Should(Say("Space quota '%s' not found", quotaName))
		...
		When("the quota exists", func() {
			...
			It("displays a table with quota names and their values and exits 0", func() {
				...
				Eventually(session).Should(Say(`service instances:\s+10`))
				Eventually(session).Should(Say(`paid service plans:\s+allowed`))
				Eventually(session).Should(Say(`app instances:\s+5`))
