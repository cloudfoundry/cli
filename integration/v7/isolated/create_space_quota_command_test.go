		...
		When("the quota does not exist", func() {
			When("no flags are provided", func() {
				It("creates the quota with the default values", func() {
					...
					Eventually(session).Should(Say("Getting space quota %s info as %s...", spaceQuotaName, userName))
					// The following defaults are correct, but the ordering and naming may change; they were copied from V6
					Eventually(session).Should(Say(`total memory limit\s+0`))
					Eventually(session).Should(Say(`instance memory limit\s+unlimited`))
					Eventually(session).Should(Say(`routes\s+0`))
					Eventually(session).Should(Say(`services\s+0`))
					Eventually(session).Should(Say(`non basic services\s+disallowed`))
					Eventually(session).Should(Say(`app instance limit\s+unlimited`))
					Eventually(session).Should(Say(`reserved route ports\s+0`))
					Eventually(session).Should(Exit(0))
				})
			})
			...
			When("all the optional flags are provided", func() {
				It("creates the quota with the specified values", func() {
					...
					Eventually(session).Should(Say("Getting space quota %s info as %s...", spaceQuotaName, userName))
					//The following defaults are correct, but the ordering and naming may change; they were copied from V6
					Eventually(session).Should(Say(`total memory limit\s+4M`))
					Eventually(session).Should(Say(`instance memory limit\s+3M`))
					Eventually(session).Should(Say(`routes\s+15`))
					Eventually(session).Should(Say(`services\s+7`))
					Eventually(session).Should(Say(`non basic services\s+allowed`))
