...
var _ = Describe("get-readiness-health-check command", func() {
	...
	Describe("help", func() {
		When("--help flag is set", func() {
			It("appears in cf help -a", func() {
				...
				Expect(session).To(HaveCommandInCategoryWithDescription("get-readiness-health-check", "APPS", "Show the type of readiness health check performed on an app"))
			})

			It("Displays command usage to output", func() {
				session := helpers.CF("get-readiness-health-check", "--help")

				Eventually(session).Should(Say("NAME:"))
				Eventually(session).Should(Say("get-readiness-health-check - Show the type of readiness health check performed on an app"))
				Eventually(session).Should(Say("USAGE:"))
				Eventually(session).Should(Say("cf get-readiness-health-check APP_NAME"))
	...
	When("the environment is not setup correctly", func() {
		It("fails with the appropriate errors", func() {
			helpers.CheckEnvironmentTargetedCorrectly(true, true, ReadOnlyOrg, "get-readiness-health-check", appName)
		})
	})
	...
	When("the environment is set up correctly", func() {
		...
		When("the input is invalid", func() {
			When("there are not enough arguments", func() {
				It("outputs the usage and exits 1", func() {
					session := helpers.CF("get-readiness-health-check")
		...
		When("the app exists", func() {
			...
			It("displays the readiness health check types for each process", func() {
				session := helpers.CF("get-readiness-health-check", appName)

				Eventually(session).Should(Say(`Getting readiness health check type for app %s in org %s / space %s as %s\.\.\.`, appName, orgName, spaceName, username))
				Eventually(session).Should(Say(`process\s+type\s+endpoint \(for http\)\s+invocation timeout\s+interval\n`))
				Eventually(session).Should(Say(`web\s+process\s+\n`))

				Eventually(session).Should(Exit(0))
			})

			// TODO: Implement these when the set-readiness-health-check-command is implemented
