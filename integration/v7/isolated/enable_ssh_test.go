	...
	When("the environment is set up correctly", func() {
		...
		When("the app does not exist", func() {
			It("displays app not found and exits 1", func() {
				...
				Eventually(session).Should(Say(`Enabling ssh support for app %s in org %s / space %s as %s\.\.\.`, invalidAppName, orgName, spaceName, userName))
		...
		When("the app exists", func() {
			...
			When("when ssh has not been enabled yet", func() {
				It("enables ssh for the app", func() {
					...
					Eventually(session).Should(Say(`Enabling ssh support for app %s in org %s / space %s as %s\.\.\.`, appName, orgName, spaceName, userName))
					Eventually(session).Should(Say("OK"))
					Eventually(session).Should(Exit(0))

					session = helpers.CF("curl", fmt.Sprintf("v3/apps/%s/ssh_enabled", helpers.AppGUID(appName)))
					Eventually(session).Should(Say(`"enabled": "%s"`, "true"))
					Eventually(session).Should(Exit(0))
				})
			})
