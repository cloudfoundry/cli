...
var _ = Describe("uninstall-plugin command", func() {
	...
	When("the plugin is installed", func() {
		BeforeEach(func() {
			...
			helpers.InstallConfigurablePlugin("configurable_plugin_v7", "banana-plugin-name-2", "1.4.3", []helpers.PluginCommand{
		...
		When("the plugin encounters an error during cleanup", func() {
			BeforeEach(func() {
				helpers.InstallConfigurablePlugin("configurable_plugin_fails_uninstall_v7", "failing-plugin", "2.0.1", []helpers.PluginCommand{
