		...
		When("the file is compiled for the correct os and architecture", func() {
			...
			When("the -f flag is given", func() {
				...
				When("the file is not a plugin", func() {
					BeforeEach(func() {
						var err error
						pluginPath, err = Build("code.cloudfoundry.org/cli/v9/integration/assets/non_plugin")
						Expect(err).ToNot(HaveOccurred())
					})
				...
				When("getting metadata from the plugin errors", func() {
					BeforeEach(func() {
						var err error
						pluginPath, err = Build("code.cloudfoundry.org/cli/v9/integration/assets/test_plugin_fails_metadata")
						Expect(err).ToNot(HaveOccurred())
					})
