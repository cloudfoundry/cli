...
var _ = SynchronizedBeforeSuite(func() []byte {
	...
	var err error
	testPluginPath, err = Build("code.cloudfoundry.org/cli/v7/integration/assets/test_plugin")
	Expect(err).ToNot(HaveOccurred())

	overrideTestPluginPath, err = Build("code.cloudfoundry.org/cli/v7/integration/assets/test_plugin_with_command_overrides")
	Expect(err).ToNot(HaveOccurred())

	panicTestPluginPath, err = Build("code.cloudfoundry.org/cli/v7/integration/assets/test_plugin_with_panic")
	Expect(err).ToNot(HaveOccurred())
})
