	...
	Describe("help", func() {
		Context("when --help flag is set", func() {
			It("Displays command usage to output", func() {
				...
				Eventually(session.Out).Should(Say("   cf bind-security-group SECURITY_GROUP ORG \\[SPACE\\] \\[--lifecycle \\(running \\| staging\\)\\]"))
				...
				Eventually(session.Out).Should(Say("   --lifecycle      Lifecycle phase the group applies to \\(Default: running\\)"))
