...
var _ = Describe("v3-list-droplets command", func() {
	...
	Describe("help", func() {
		Context("when --help flag is set", func() {
			It("Displays command usage to output", func() {
				session := helpers.CF("v3-list-droplets", "--help")

				Eventually(session.Out).Should(Say("NAME:"))
				Eventually(session.Out).Should(Say("v3-list-droplets - \\*\\*EXPERIMENTAL\\*\\* List droplets of an app"))
				Eventually(session.Out).Should(Say("USAGE:"))
				Eventually(session.Out).Should(Say("cf v3-list-droplets APP_NAME"))
	...
	Context("when the app name is not provided", func() {
		It("tells the user that the app name is required, prints help text, and exits 1", func() {
			session := helpers.CF("v3-list-droplets")
	...
	Context("when the environment is not setup correctly", func() {
		Context("when no API endpoint is set", func() {
			...
			It("fails with no API endpoint set message", func() {
				session := helpers.CF("v3-list-droplets", appName)
