	...
	Context("when the environment is set up correctly", func() {
		...
		Context("with existing apps", func() {
			...
			It("displays apps in the list", func() {
				...
				Eventually(session).Should(Say("%s\\s+started\\s+web:1/1, console:0/0, rake:0/0, worker:0/0\\s+%s\\.%s", appName1, appName1, domainName))
				Eventually(session).Should(Say("%s\\s+started\\s+web:1/1, console:0/0, rake:0/0, worker:0/0\\s+%s\\.%s", appName2, appName2, domainName))
			...
			Context("when one app is stopped", func() {
				...
				It("displays app as stopped", func() {
					...
					Eventually(session).Should(Say("Getting apps in org %s / space %s as %s\\.\\.\\.", orgName, spaceName, userName))
					Eventually(session).Should(Say("name\\s+requested state\\s+processes\\s+routes"))
					Eventually(session).Should(Say("%s\\s+stopped\\s+web:0/1, console:0/0, rake:0/0, worker:0/0\\s+%s\\.%s", appName1, appName1, domainName))
					Eventually(session).Should(Say("%s\\s+started\\s+web:1/1, console:0/0, rake:0/0, worker:0/0\\s+%s\\.%s", appName2, appName2, domainName))
