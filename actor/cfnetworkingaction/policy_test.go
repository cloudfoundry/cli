...
var _ = Describe("Policy", func() {
	...
	Describe("NetworkPoliciesBySpaceAndAppName", func() {
		...
		When("getting the spaces by guids fails", func() {
			BeforeEach(func() {
				fakeCloudControllerClient.GetSpacesReturns([]ccv3.Space{}, []string{"GetSpacesWarning"}, errors.New("banana"))
			})
	...
	Describe("NetworkPoliciesBySpace", func() {
		...
		It("lists policies", func() {
			...
			Expect(fakeCloudControllerClient.GetApplicationsCallCount()).To(Equal(1))
			Expect(fakeCloudControllerClient.GetApplicationByNameAndSpaceCallCount()).To(Equal(0))

			Expect(fakeNetworkingClient.ListPoliciesCallCount()).To(Equal(1))
			Expect(fakeNetworkingClient.ListPoliciesArgsForCall(0)).To(ConsistOf("appAGUID", "appBGUID", "appCGUID"))

			Expect(fakeCloudControllerClient.GetApplicationsCallCount()).To(Equal(1))
			Expect(fakeCloudControllerClient.GetApplicationsArgsForCall(0)).To(ConsistOf("appBGUID", "appCGUID"))

			Expect(fakeCloudControllerClient.GetSpacesCallCount()).To(Equal(1))
			Expect(fakeCloudControllerClient.GetSpacesArgsForCall(0)).To(ConsistOf("spaceAGUID", "spaceCGUID"))

			Expect(fakeCloudControllerClient.GetOrganizationsCallCount()).To(Equal(1))
			Expect(fakeCloudControllerClient.GetOrganizationsArgsForCall(0)).To(ConsistOf("orgAGUID", "orgCGUID"))
		})
		...
		When("getting the applications fails", func() {
			BeforeEach(func() {
				fakeCloudControllerClient.GetApplicationsReturns([]ccv3.Application{}, []string{"GetApplicationsBySpaceWarning"}, errors.New("banana"))
			})
