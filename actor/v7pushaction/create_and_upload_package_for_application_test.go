...
var _ = Describe("CreateAndUploadPackageForApplication", func() {
	var (
		actor           *Actor
	...
	JustBeforeEach(func() {
		events = EventFollower(func(eventStream chan<- Event) {
			returnedPushPlan, warnings, executeErr = actor.CreateAndUploadPackageForApplication(paramPlan, eventStream, fakeProgressBar)
		})
	})
	...
	Describe("package upload", func() {
		When("docker image is provided", func() {
			BeforeEach(func() {
				paramPlan.DockerImageCredentialsNeedsUpdate = true
				paramPlan.DockerImageCredentials.Path = "some-docker-image"
				paramPlan.DockerImageCredentials.Password = "some-docker-password"
				paramPlan.DockerImageCredentials.Username = "some-docker-username"
