...
var _ = Describe("UpdateRoutesForApplicationWithRandomRoute", func() {
	var (
		actor       *Actor
	...
	JustBeforeEach(func() {
		events = EventFollower(func(eventStream chan<- *PushEvent) {
			_, warnings, executeErr = actor.UpdateRoutesForApplicationWithRandomRoute(paramPlan, eventStream, nil)
		})
	})

	When("creating a random route", func() {
		BeforeEach(func() {
			paramPlan.SkipRouteCreation = false
			paramPlan.RandomRoute = true
		})
