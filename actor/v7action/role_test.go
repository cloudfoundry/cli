...
var _ = Describe("Role Actions", func() {
		...
		Context("the user is not found", func() {
			When("The user is not a client", func() {
				...
				It("returns a user not found error and warnings", func() {
					...
					Expect(executeErr).To(MatchError(actionerror.UserNotFoundError{Username: userNameOrGUID, Origin: userOrigin}))
				})
			})
	...
	Describe("GetRoleGUID", func() {
		...
		BeforeEach(func() {
			roleType = constant.SpaceDeveloperRole
			queryKey = ccv3.SpaceGUIDFilter
			orgOrSpaceGUID = "space-guid"
			userGUID = "user-guid"
		})

		JustBeforeEach(func() {
			roleGUID, warnings, executeErr = actor.GetRoleGUID(queryKey, orgOrSpaceGUID, userGUID, roleType)
		})
		...
		When("the role exists and no errors occur", func() {
			...
			When("the role is an org role", func() {
				BeforeEach(func() {
		...
		When("the role does not exist and no errors occur", func() {
			...
			It("it gets an empty string and no errors", func() {
				passedRolesQuery := fakeCloudControllerClient.GetRolesArgsForCall(0)
				Expect(passedRolesQuery).To(Equal(
					[]ccv3.Query{
						{
	...
	Describe("DeleteOrgRole", func() {
		...
		When("the API call to delete the org role returns an error", func() {
			BeforeEach(func() {
				...
				fakeCloudControllerClient.DeleteRoleReturnsOnCall(0,
					ccv3.JobURL(""),
					ccv3.Warnings{"delete-org-role-warning"},
					errors.New("delete-org-role-error"),
				)
			})

			It("it returns an error and warnings", func() {
				Expect(fakeCloudControllerClient.DeleteRoleCallCount()).To(Equal(1))
				Expect(warnings).To(ConsistOf("get-users-warning", "get-roles-warning", "delete-org-role-warning"))
				Expect(executeErr).To(MatchError("delete-org-role-error"))
			})
		})
	})
