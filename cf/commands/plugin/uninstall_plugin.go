...
func (cmd *PluginUninstall) SetDependency(deps commandregistry.Dependency, pluginCall bool) commandregistry.Command {
	...
	RPCService, err := rpcService.NewRpcService(deps.TeePrinter, deps.TeePrinter, v3Config, deps.RepoLocator, rpcService.NewCommandRunner(), deps.Logger, cmd.ui.Writer(), server)
	if err != nil {
