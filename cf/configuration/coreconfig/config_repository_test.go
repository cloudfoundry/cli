...
import (
	...
	"code.cloudfoundry.org/cli/v8/cf/configuration/configurationfakes"
...
var _ = Describe("Configuration Repository", func() {
	...
	Describe("UserGUID", func() {
		...
		Context("with a valid client access token", func() {
			BeforeEach(func() {
				config.SetAccessToken(AccessTokenForClientUsers)
			})
	...
	Describe("Username", func() {
		...
		Context("with a valid client access token", func() {
			...
			It("returns the client id", func() {
				Expect(config.Username()).To(Equal("potato-face"))
			})
		})
